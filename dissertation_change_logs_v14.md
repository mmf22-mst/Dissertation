...
Archival change logs for the dissertation overview document. These were extracted from **dissertation_overview_and_paper_outlines_v14.md** to keep the design document focused on the current design. The two files are paired: this log is not valid against a different revision of the overview.

---

### Revision 14 change log

Injection source and alignment-rate redesign.

1. **"Treatment is the package" principle adopted.** B1 uses `bfo-core.owl` as the BFO project ships it (36 classes, 40 relations). B2 uses `Core.rdf` with its full import chain resolved: classic `bfo.owl` (36 classes, 64 relations) + IOF construct-namespace vocabulary (91 classes incl. `BFO_0000144`, 83 object properties). The BFO layers differ by 24 relation variants; this is part of the treatment. The earlier common-BFO-base principle was explored and rejected: IOF's own axioms use 12 of the 24 classic-build-only relations across 29 sites in core class definitions (including `PlanSpecification`, `Agreement`, `BusinessProcess`, `Organization`, and several IOF property declarations); substituting `bfo-core.owl` would break those references and create a degraded IOF no practitioner would encounter.
2. **Alignment-rate partition rule replaced by a single-SPARQL priority cascade.** The query applies across all conditions: `BFO_0000144` → IOF-aligned; `iof-constr:` → IOF-aligned; other `obo:BFO_` → BFO-only-aligned; else unaligned. No condition-specific class inventories. The `BFO_0000144` carve-out is necessary because this class has a BFO-namespace IRI but is exclusively IOF-contributed (absent from both `bfo-core.owl` and classic `bfo.owl`; sourced by IOF from BFO 2.0). Without it, a domain class under `BFO_0000144` would be misclassified as BFO-only-aligned.
3. **Contamination signal added.** Nonzero BFO-aligned or IOF-aligned rates in conditions not given that vocabulary (B0 for either, B1 for IOF) are reported as contamination from pre-training. Threshold for discussion: > 5% of domain classes.
4. **Pre-campaign BFO build diff added** to the pinned-release set and Paper 1 methods (§2.3): confirms class-level identity between `bfo-core.owl` and classic `bfo.owl`, with the difference confined to 24 relation variants.
5. **Alignment-rate query implemented** (`alignment_rate.sparql`): validated against IOF Core 202603 parse across eight critical topologies including the `BFO_0000144` carve-out, IOF bypass, contamination scenarios, and the unaligned case.
6. **Shared research assets** updated: item 5 is now a single alignment-rate query (was two); item 6 names all three pinned files with their roles and commits to the pre-campaign diff.
7. **Portability table** updated: "base ontologies for Factor B" → "source files each grounding condition uses, with their import chains resolved"; notes that the `BFO_0000144` carve-out may need updating for a different mid-level.
8. **Cross-cutting risks** updated: the B0 pretraining row now references the contamination signal and the 5% discussion threshold.
9. **Todo item 2 closed:** design settled and query implemented.
10. **No hypotheses, factors, research questions, or other instruments changed.**
