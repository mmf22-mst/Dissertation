...
genre-map   JSON mapping filename → genre label (one of the six v19 genre codes).
...
v20 (overview Part I §6.2(c)): at scoring time the CQ patterns match the
*normalised scoring label* (label_normalisation.py: CamelCase/underscore
split, lower-case, punctuation stripped, whitespace collapsed, head noun
singularised), never the raw rdfs:label.  A pattern term written in
plural or with punctuation would therefore never match.  The report
flags every pattern term that is not already in normalised form
("non_normalised_terms") so the CQ set can be fixed before it is frozen.
...
# v19: six genres.  Process Maps, Corrective Action Reports, Strategic
# Documents, ERP and MES were dropped from the experimental corpus.  CQs
# tagged with a dropped genre are reported (see dropped_genre_tags) so they
# can be pruned or rewritten during the CQ re-scope (todo task 1).
GENRE_CODES = {
    "DPB": "DPBPS",
    "CMD": "Command Media",
    "FME": "FMEAs",
...
DROPPED_GENRE_CODES = {"PRC", "CAR", "STR", "ERP", "MES"}

# Question types forming the primary held-out outcome (v19).  Their SPARQL
# must join at least two label-matched classes through an object property.
PRIMARY_QTYPES = {"relational", "multi-hop"}

# Predicates that do not count as a structural join between classes.
_NON_JOIN_PREDICATES = {
    "rdfs:label", "rdf:type", "a", "rdfs:subClassOf", "rdfs:comment",
...
def dropped_genre_tags(rec: "CQRecord") -> list[str]:
    """Genre codes on this CQ that are not in the v19 genre set."""
    return [g for g in rec.genre_tags if g in DROPPED_GENRE_CODES]


def needs_join_review(rec: "CQRecord") -> bool:
    """
    Heuristic flag for primary-stratum CQs whose SPARQL may not join
    classes through an object property.

    Flags a relational or multi-hop CQ when it has fewer than two label
    patterns, or when no triple pattern uses a predicate outside the
    labelling/typing/subsumption vocabulary (a variable predicate such as
    ?p, a prefixed property, or a property path counts as a join).  This
    is a screening aid for the task-1 rewrite, not a proof of correctness.
    """
    if rec.qtype not in PRIMARY_QTYPES:
        return False
    if len(rec.patterns) < 2:
        return True
    body = re.sub(r"#.*", "", rec.sparql)
    body = re.sub(r"FILTER\s*\(.*?\)\s*(?=[.}\n])", " ", body, flags=re.S | re.I)
    body = re.sub(r'"[^"]*"', '""', body)
    for m in re.finditer(r"\?\w+\s+(\S+)\s+\?\w+", body):
        pred = m.group(1)
        if pred.startswith("?"):
            return False
        if pred not in _NON_JOIN_PREDICATES:
            return False
    return True
...
class CQRecord:
    ...
    zero_hit: bool = False
    patched_sparql: str = ""
    # v20: (term, normalised form) for pattern terms not in normalised form
    non_normalised_terms: list[tuple[str, str]] = field(default_factory=list)
...
def flag_non_normalised_terms(records: list[CQRecord]) -> None:
    """
    v20: mark pattern terms that would never match a normalised scoring
    label.  Uses the pinned rule from label_normalisation; if that module
    is unavailable (standalone use) the check is skipped with a warning.
    """
    try:
        from label_normalisation import check_pattern_terms
    except ImportError:
        try:
            from .label_normalisation import check_pattern_terms  # type: ignore
        except ImportError:
            print("WARNING: label_normalisation not importable; skipping "
                  "normalised-form check.", file=sys.stderr)
            return
    for rec in records:
        terms = [t for p in rec.patterns for t in p.alternates]
        rec.non_normalised_terms = check_pattern_terms(terms)
...
def build_report(records: list[CQRecord]) -> dict:
    ...
    summary = {
        ...
        "primary_cqs": sum(1 for r in records if r.qtype in PRIMARY_QTYPES),
        "dropped_genre_cqs": sum(1 for r in records if dropped_genre_tags(r)),
        "join_review_cqs": sum(1 for r in records if needs_join_review(r)),
        ...
        "non_normalised_term_cqs": sum(1 for r in records if r.non_normalised_terms),
    }
    ...
    for rec in records:
        detail = {
            ...
            "qtype": rec.qtype,
            "primary": rec.qtype in PRIMARY_QTYPES,
            "dropped_genre_tags": dropped_genre_tags(rec),
            "needs_join_review": needs_join_review(rec),
            ...
            "non_normalised_terms": [
                {"term": t, "normalised": n} for t, n in rec.non_normalised_terms
            ],
        }
...
def print_summary(records: list[CQRecord]) -> None:
    ...
    print(f"{'='*70}")
    dropped = [r for r in records if dropped_genre_tags(r)]
    join_review = [r for r in records if needs_join_review(r)]
    print(f"Total CQs:              {total}")
    print(f"Primary CQs:            {sum(1 for r in records if r.qtype in PRIMARY_QTYPES)}"
          f"  (relational + multi-hop)")
    print(f"Dropped-genre CQs:      {len(dropped)}  (prune or rewrite — v19 genres)")
    print(f"Join-review CQs:        {len(join_review)}  (primary CQs that may lack an object-property join)")
    print(f"Zero-hit CQs:           {len(zero_hit)}  (need manual review)")
    nn = [r for r in records if r.non_normalised_terms]
    print(f"Non-normalised terms:   {len(nn)} CQs  (fix before freeze — v20 rule)")
...
def main():
    ...
    # Step 4: report
    flag_non_normalised_terms(records)   # v20
