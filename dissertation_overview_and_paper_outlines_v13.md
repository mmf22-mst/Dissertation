...
**Revision 13 — E-code simplification: all six Tier A/B codes (E1–E6) removed from the design, along with the source-linked codebook, the three B2 baseline-exclusion lists, the scoring-layer apparatus, rules R1–R9, descriptive indicators I1–I5, and the codebook crosswalk. Replaced by two SPARQL-based alignment-rate manipulation checks (BFO-only alignment rate and IOF alignment rate) with a priority-based partition rule. The pitfall-overlap audit is simplified to OOPS!-to-OntoQA only. RQ2.4's structural quality vector is now grounding-agnostic predictors only, estimable on all 60 ontologies. The codebook is retained in the project archive for possible separate publication. Prior revision history: v12 reconciled against codebook v12; v11.4 D1 iteration on sub-ontologies with anchored re-integration; v11.3 three-allotment prompt decomposition; v11.2 corpus presentation order and deterministic windowing; v11.1 OOPS! retained in feedback loop with pitfall-overlap audit; v11 loop-coupled instruments identified and quarantined.**


*Body text presents the current design without backward references to prior revisions. Change logs are in the companion file **dissertation_change_logs_v13.md**.*
...
Two alignment rates verify that Factor B produced the intended vocabulary usage. Both are SPARQL queries over the asserted `rdfs:subClassOf` hierarchy, run on all three grounding levels.

**Partition rule.** For each domain class C, walk the asserted `rdfs:subClassOf` chain upward:

- **IOF-aligned:** the chain reaches an IOF Core class before (or without) reaching a bare BFO class.
- **BFO-only-aligned:** the chain reaches a BFO class without passing through any IOF Core class.
- **Unaligned:** the chain reaches neither.

The three categories partition the domain classes exhaustively. Every IOF Core class is itself a descendant of a BFO class, so a naïve "has a BFO ancestor" query would count IOF-aligned classes too; the priority rule prevents this.
...
| B0 | ≈ 0 | ≈ 0 | ≈ all |
| B1 | high | ≈ 0 | remainder |
| B2 | low | high | remainder |

The B1-vs-B2 contrast is visible: B1 has high BFO-only alignment, B2 has high IOF alignment, and the BFO-only rate in B2 reveals how often the generator bypassed IOF's mid-level vocabulary and attached directly to BFO despite IOF being available.
...
5. **The two alignment-rate queries.** SPARQL queries implementing the BFO-only and IOF alignment-rate partition rule (§5.3), versioned and pinned.
6. **The pinned-release set.** `bfo-core.owl` (BFO 2020); IOF Core `Core.rdf` release 202603; ISO/IEC 21838-1 and -2:2021. File hashes recorded in the seed register.
...
| The base ontologies for Factor B | Alignment-rate queries (generic SPARQL) |
...
2. An evaluation battery combining established structural metrics (OntoQA, OOPS!), corpus-grounded coverage measures, and a two-dimensional alignment-rate manipulation check, applied without a reference ontology and at every iteration round.
...
4. Released artefacts: generators, battery scripts, CQ set, and all 60 × 10 ontology-states.
...
3.6 **Manipulation checks.** BFO-only alignment rate and IOF alignment rate, with the priority-based partition rule (Part I §5.3).
