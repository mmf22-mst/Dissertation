# Windowing Selection Rule and Patch-Merge Semantics — Specification

Companion to dissertation_overview_and_paper_outlines_v15.md, Part I §5
(context-overflow policy) and Paper 1 §4.3–4.4.

Implements task 4a of dissertation_todo_v15.md.
...
During iteration (R1–R9), the feedback round operates on the *full*
ontology at both D levels. Under D0 this means the accumulated artefact;
under D1 this means each sub-ontology individually (which is smaller
and less likely to trigger windowing) and the integrated artefact
(which is the same size as D0's and may trigger windowing during the
integration pass).

If windowing fires during an iteration round, the same selection rule,
patch-merge semantics, and logging apply. The `phase` field distinguishes
construction from iteration in the log.
