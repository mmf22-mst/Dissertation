...
**Revision 17 — Salient-term coverage and genre coverage balance reclassified from held-out to coupled; CQ answerability retained as sole held-out instrument. H3 mediation interpretation clarified: residual direct path as retrieval-structural instrument gap. Prior revision history: v16 deterministic integration for D1; v15 OntoQA reclassified as coupled, pitfall-overlap audit removed; v14 injection source and alignment-rate redesign; v13 E-code simplification; v12 reconciled against codebook v12; v11.4 D1 iteration on sub-ontologies with anchored re-integration; v11.3 three-allotment prompt decomposition; v11.2 corpus presentation order and deterministic windowing; v11.1 OOPS! retained in feedback loop with pitfall-overlap audit; v11 loop-coupled instruments identified and quarantined.**


*Body text presents the current design without backward references to prior revisions. Change logs are in the companion file **dissertation_change_logs_v17.md**.*

The evaluation architecture is fully automated and reference-free. No gold reference ontology, no hand-annotated triple sample, and no expert-adjudicated root cause benchmark. Instruments are either established tools with published validation (OOPS!, OWL reasoners, OntoQA-family structural metrics) or corpus-grounded coverage measures. The human contribution is the hand-authored competency question set, performed once and independent of the number of experimental runs.
...
| Level | Label | Supplied to the generator | Source file |
|---|---|---|---|
| B0 | Ungrounded | No upper or mid-level ontology; instructions on OWL 2 DL syntax only | — |
| B1 | BFO-grounded | BFO 2020 class and relation inventory, with definitions | `bfo-core.owl` (36 classes, 40 relations) |
| B2 | IOF-grounded | IOF Core (which imports BFO) class and relation inventory, with definitions | `Core.rdf` with import chain resolved: classic `bfo.owl` (36 classes, 64 relations) + IOF construct-namespace vocabulary (91 classes incl. `BFO_0000144`, 83 object properties) |
...
**Integration procedure for D1.** Sub-ontologies are merged by a fully deterministic two-stage procedure fixed in advance: (1) label normalisation, IRI reconciliation, and raw union; (2) LogMap alignment-based reconciliation — LogMap identifies equivalent classes and properties across sub-ontologies that use different IRIs, and a deterministic rule (lexicographically first IRI) selects the canonical form, with owl:equivalentClass declarations added for transparency. AML is run over the same inputs as an audit comparison; agreement between the two matchers is reported (Paper 1 §8.1). No LLM call is involved in integration. Sub-ontology generation plus integration together constitute the R0 state for D1, so that R0 denotes "the first complete artefact, before any feedback" at both D levels.
...
A single SPARQL query over the asserted `rdfs:subClassOf` hierarchy classifies every domain class by alignment, run on all three grounding levels. For each domain class C, the query walks the asserted `rdfs:subClassOf` chain upward and applies the following priority cascade at the first ancestor whose IRI matches a rule:

1. **`obo:BFO_0000144`** → **IOF-aligned.** This class has a BFO-namespace IRI but is exclusively IOF-contributed: it is absent from both `bfo-core.owl` and classic `bfo.owl`, and was sourced by IOF from BFO 2.0. It is the only BFO-namespace class with this property.
2. **Any `iof-constr:` IRI** → **IOF-aligned.**
3. **Any other `obo:BFO_` IRI** → **BFO-only-aligned.**
4. **No match** → **Unaligned.**

The three categories partition domain classes exhaustively. The `BFO_0000144` carve-out is necessary because without it, a domain class under `BFO_0000144` (process profile) would be misclassified as BFO-only-aligned despite the model having learned about that class solely from the IOF injection. The query requires no condition-specific class inventories.

**Expected pattern and contamination signal.**

| | BFO-only aligned | IOF aligned | Unaligned |
|---|---|---|---|
| B0 | ≈ 0 (contamination if > 0) | ≈ 0 (contamination if > 0) | ≈ all |
| B1 | high | ≈ 0 (contamination if > 0) | remainder |
| B2 | low (bypass rate) | high | remainder |

The contamination column identifies cases where the LLM imported upper-level vocabulary from its pre-training data rather than from the injection. B0 with BFO-aligned or IOF-aligned > 0 means the model used BFO or IOF IRIs it was never shown. B1 with IOF-aligned > 0 means the model used IOF vocabulary despite receiving only BFO. B2 with high BFO-only-aligned is not contamination but a measure of how often the generator bypassed IOF's mid-level vocabulary and attached directly to BFO despite IOF being available. Contamination counts are reported descriptively per condition. If any condition shows contamination rates above 5% of domain classes, the report discusses whether pre-training exposure to BFO/IOF constitutes a threat to the manipulation.

**Coupling.** Both alignment rates are reported by the conformance channel in the feedback payload, so the model is partly told to improve the quantities used to verify the manipulation. Alignment rates at R0 are the clean manipulation check; later rounds show how the manipulation is maintained under feedback.

**Scope limit.** Alignment rates measure vocabulary *presence*, not *correctness*. A high rate is compatible with misuse of the categories — placing a process under a continuant class, for example, would still count as aligned. Any condition-linked misuse effect is invisible at this level and becomes visible only through downstream measures in Papers 2 and 3. This is stated as a limitation and is the trade-off of dropping the BFO-specific modelling checks (six formally decidable E-codes, derived from BFO 2020 with source-linked provenance) that earlier revisions carried. The codebook and detectors are retained in the project archive for possible separate publication or reinstatement.
...
- **Co-occurrence-anomalous entailments (deferred / drop candidate):** entailed subsumptions between classes whose corpus co-occurrence is at or near zero, flagged as candidate spurious inferences and ranked by anomaly score. This instrument is threshold-dependent and has no oracle. It is exploratory only, no hypothesis is attached, no other instrument depends on it, and it is not on the critical path. Deferred from the initial implementation; can be added after the pilot if the committee requests it.

#### 5.5 The full instrument inventory

| Family | Instruments | Scope | Loop status |
|---|---|---|---|
| Manipulation check | BFO-only alignment rate; IOF alignment rate | All levels | Coupled (conformance channel) |
| Corpus grounding | Salient-term coverage; genre coverage balance | All levels | **Coupled** (coverage channel) |
| Corpus grounding | CQ answerability | All levels | Held out |
| Logical quality | Consistency; unsatisfiable classes; OWL 2 DL conformance | All levels | **Coupled** |
| Logical quality (deferred) | Co-occurrence-anomalous entailments | All levels | Held out |
...
All instruments run across all three grounding levels.
...
Five limits, stated plainly and carried into every limitations section:
...
5. **Part of the battery is coupled to the iteration loop.** See §6.7. For the coupled instruments, improvement across rounds is partly definitional, and only the R0 contrast is a clean between-condition comparison.
...
**What is in the loop.** The feedback payload is six items: the reasoner report (global consistency and the list of unsatisfiable classes), the OWL 2 DL profile report, the OOPS! pitfall report, the conformance report, the salient-term coverage report (missing corpus terms by genre), and the genre coverage balance report (per-genre coverage entropy). These are the corrective signal; removing any of them would leave iteration with less to act on and would make H1d uninterpretable in the opposite direction.
...
**Why salient-term coverage and genre coverage balance enter the loop (v17).** The same standing-priority logic applies. Both instruments produce directly actionable feedback: the coverage report tells the LLM which corpus terms are missing, and the genre balance report tells it which genres are underrepresented. A practitioner who had these signals would use them. Withholding them would depress artefact quality for Papers 2 and 3 to protect secondary Paper 1 outcomes — the same trade the OOPS! decision (v11.1) rejected.

**Why CQ answerability stays out.** Holding out CQs is well-motivated on two independent grounds. First, CQ feedback is not actionable for the LLM: acting on it would require reading SPARQL queries, inferring what structural patterns each expects, and reverse-engineering what the ontology is missing — a much harder inference chain than "add this missing term" or "fix this pitfall." Second, it preserves the cleanest independent measure of whether iteration works at the ontology level. Because the CQ set is small (50–100 questions), feeding it back would also risk overfitting: the LLM might add shallow structures that satisfy SPARQL patterns without genuinely modelling the domain.

**Consequences for inference.** The coupled measures are global consistency, unsatisfiable-class count, OWL 2 DL conformance, OOPS! pitfall counts, OntoQA structural profile, the two alignment rates, salient-term coverage, and genre coverage balance. For these:
...
**Why no pitfall suppression.** Earlier revisions carried a pitfall-overlap audit that would have suppressed OOPS! pitfalls whose remediation mechanically moves an OntoQA metric. Reclassifying OntoQA as coupled removes the need: there is no held-out metric to protect, so the full OOPS! report is fed back. This follows the same standing priority as the decision to keep OOPS! in the loop (revision 11.1): the generator should receive every free automated signal a practitioner would use, and the coupling cost is absorbed rather than avoided.

### 7. Hypothesis chain

**H1 (Paper 1).**
- H1a — Grounding B affects ontology quality. Tested across all three levels on the grounding-agnostic instruments. The prediction is that grounded ontologies outperform ungrounded ones. Coupled measures (consistency, OWL 2 DL conformance, OOPS! pitfall counts, OntoQA structural profile, alignment rates) contribute at R0 only.
...
**Scope restriction on H1d–H1f.** Confirmatory tests of the three round-related hypotheses are run on CQ answerability (the sole held-out instrument, §6.7) in Paper 1, and on KG fidelity and RCA accuracy in Papers 2 and 3. The coupled instruments are reported alongside as process evidence, clearly labelled, and are not counted toward support for any of the three.

**Primary outcome per hypothesis.** CQ answerability is the primary outcome for H1a, H1b, H1c, H1d, H1e, and H1f — the sole held-out instrument and therefore the only Paper 1 measure that provides independent confirmatory evidence across all hypotheses. Salient-term coverage and genre coverage balance, now coupled, are secondary outcomes reported with effect sizes and intervals; they are confirmatory at R0 and compliance evidence thereafter. Every other instrument is secondary and reported the same way.
...
5. **The alignment-rate query.** A single SPARQL query implementing the priority cascade (§5.3), with the `BFO_0000144` carve-out hardcoded and commented. Versioned and pinned.
6. **The pinned-release set.** `bfo-core.owl` (BFO 2020, B1 injection source); classic `bfo.owl` as cached in IOF Core release 202603 (B2, via import chain); IOF Core `Core.rdf` release 202603 (B2 injection source). File hashes recorded in the seed register. A pre-campaign file-level diff between `bfo-core.owl` and classic `bfo.owl` confirms that the class-level content is identical and the difference is confined to 24 relation variants; this diff is reported in Paper 1's methods appendix. ISO/IEC 21838-1 and -2:2021 are reference documents, not injection sources.
7. **The model manifest.** Generator weights and revision, serving runtime and version, quantisation if any, context window, and the same for the NLI verifier (Paper 2), the NLI answer matcher (Paper 3), and the embedding model used in salient-term matching. Pinned for the duration of the campaign.
8. **The entailment-based triple verifier** (Paper 2).
9. **The held-out root-cause label set** (Paper 3), mined automatically from corrective-action statements and redacted from the retrieval corpus.
...
| The source files each grounding condition uses, with their import chains resolved | Alignment-rate query (generic SPARQL; `BFO_0000144` carve-out may need updating for a different mid-level) |
...
2. An evaluation battery combining established structural metrics (OntoQA, OOPS!), corpus-grounded coverage measures, and a priority-cascade alignment-rate manipulation check with built-in contamination detection, applied without a reference ontology and at every iteration round.
3. **An end-to-end empirical trace of whether upstream ontology design — and the iteration investment — survives to downstream task performance**, including the practitioner-relevant question of whether grounding substitutes for iteration.
4. Released artefacts: generators, battery scripts (including the alignment-rate query), CQ set, and all 60 × 10 ontology-states.
...
| Cross-genre defects invisible to a per-sub-ontology feedback loop | Feedback computed on the integrated artefact and routed to all involved sub-ontologies (§4.4a) |
...
| Paper 1 generation, D1 surcharge | Additional calls | Per round, *G* sub-ontology calls rather than one; with six genres, 6× D0's per-round LLM cost; integration is deterministic and adds no LLM calls | Genre count |
...
2.3 Realist foundations: BFO as ISO/IEC 21838-2; IOF Core as a BFO-conformant manufacturing mid-level; CCO as sibling, with the absence of a vetted mapping stated. **Release reconciliation:** two BFO OWL builds exist — `bfo-core.owl` (canonical, 40 relations) and classic `bfo.owl` (imported by IOF, 64 relations); a pre-campaign diff confirms identical class-level content, with the difference confined to 24 relation variants. B1 uses `bfo-core.owl`; B2 uses IOF Core with its import of classic `bfo.owl`. The 24 additional relations are part of the IOF package: IOF's own axioms reference 12 of them across 29 sites in core class definitions.
...
3.3 **Competency question answerability.** 50–100 hand-authored CQs with automated SPARQL scoring; stratified by question type.
...
3.5 **Logical quality.** OWL 2 DL conformance; consistency and satisfiability. Co-occurrence-anomalous entailments deferred (exploratory, no hypothesis attached; can be added post-pilot).
3.6 **Manipulation checks.** BFO-only alignment rate and IOF alignment rate, with the single-SPARQL priority cascade and `BFO_0000144` carve-out (Part I §5.3). Contamination signal reported for B0 and B1.
3.7 **Consensus induction** (R9 only), with the explicit non-truth caveat.
...
4.2 **Injection protocol for Factor B, and the three-allotment prompt.** The generation prompt has three variable parts, and only one of them is the manipulation:
...
**Injection sources and the "treatment is the package" principle.** Each grounding condition receives the package its source organisation ships, unmodified. B1 uses `bfo-core.owl` — the canonical file the BFO project distributes (36 classes, 40 relations). B2 uses IOF Core `Core.rdf` with its full import chain resolved: classic `bfo.owl` (36 classes, 64 relations) plus IOF's construct-namespace vocabulary (91 classes including `BFO_0000144`, 83 object properties). The BFO content accessible through B1 and B2 differs: classic `bfo.owl` adds 24 temporal relation variants ("at all times", "proper", and "some time proper" forms) that `bfo-core.owl` does not contain. This difference is part of the treatment, not a confound. IOF was designed and tested against classic `bfo.owl`, and its own axioms depend on those 24 additional relations: 12 of them appear in 29 axiom references across core IOF class definitions (including `PlanSpecification`, `Agreement`, `BusinessProcess`, `Organization`, and several IOF property declarations). Substituting `bfo-core.owl` into B2's import chain would break those references and create a degraded IOF that no practitioner would encounter. A practitioner who chooses "BFO only" reaches for `bfo-core.owl`; a practitioner who chooses "IOF Core" gets `Core.rdf` with its full import chain.

**Injection rendering.** B1 and B2 injections are produced by a deterministic renderer that extracts class and relation labels with their definitions from the injection source, sorted by a fixed ordering rule (alphabetical on label, ties broken by IRI), and truncated at the pinned injection allotment with the truncation point recorded. For B2, the renderer parses the resolved import closure of `Core.rdf` (i.e. `Core.rdf` + classic `bfo.owl` + annotation vocabulary) and renders the BFO and IOF content as a single integrated block, matching what a practitioner would see in a tool like Protégé after loading IOF Core. B2 is therefore a *summarised* IOF Core condition and is described as such throughout, not as full IOF Core.

**Total prompt length is not equalised, and B0 is not padded.** With the other two allotments fixed, the residual difference in total prompt length across B is exactly the treatment: B2's prompt is longer because it contains more vocabulary. Padding B0 with filler would not control for anything — it would add a third manipulation and dilute attention without contributing content. What remains is attention dilution over a longer prompt, which cannot be designed away and is recorded in §10 as a stated limit rather than a mitigated one. The concern is empirically grounded: Levy, Jacoby & Goldberg (ACL 2024) show that LLM reasoning degrades with input length alone — even when the added content is relevant — and Liu et al. (TACL 2024) document positional attention bias that concentrates on prompt boundaries. Both effects operate at the scale of the B0/B2 length difference, and padding would worsen both rather than control for either.
...
- 4.5.2 *Decoding regime.* Greedy decoding; seed variance estimates sensitivity to corpus presentation order — a meaningful quantity reported in §7.3.
- 4.5.3 *Execution determinism on self-hosted hardware.* Fixed GPU allocation, deterministic kernel flags, batch size 1, prefix caching disabled. Cost is throughput, accepted for a campaign of 60 runs.
- 4.5.4 *Determinism audit.* Three runs of one configuration under identical seed, outputs hashed. Matching hashes confirm bitwise reproducibility. Non-matching hashes trigger the documented fallback: divergence rate measured and reported.
...
5.1 **Manipulation checks.** BFO-only alignment rate and IOF alignment rate by condition and round; expected ordering B2 > B1 > B0 for total alignment, with the BFO-only / IOF partition distinguishing B1 from B2.
5.2 **Primary outcomes** (pre-registered). The undefined "logical soundness composite" of earlier revisions is dissolved: composites require a formula, and an unspecified one is a researcher-degrees-of-freedom hole in a pre-registered design. Outcomes are named individually.
...
The R0-only restriction on the last group follows from loop coupling (Part I §6.7), not from any doubt about the instruments themselves.

5.3 **Secondary and process outcomes:** OntoQA structural profile; cumulative token cost and wall-clock time; feedback payload volume. **D1 integration descriptives:** integration loss — the battery delta between the union of the sub-ontologies and the integrated artefact, computed at every round on the grounding-agnostic instruments, which quantifies how much quality the deterministic merge loses and whether that varies with grounding. Salient-term coverage on a sub-ontology is scored against that genre's inventory, not the whole-corpus inventory. Round-to-round delta rate (the proportion of axioms that change in the integrated artefact between consecutive rounds, caused entirely by sub-ontology iteration, not merge variance); IRI stability across rounds (including LogMap alignment stability as sub-ontologies evolve); and the defect-routing split between single-source and multi-source defects. **Position-gradient descriptives:** per-genre salient-term coverage by processing position, which tests the recency/primacy mechanism behind H1b more directly than the entropy-based genre coverage balance does; straddling-chunk counts; and windowed-chunk counts by position and genre, with the proportion of the artefact visible. **Loop-coupled trajectories** (consistency, unsatisfiable-class rate, OWL 2 DL conformance, OOPS! pitfall counts, OntoQA structural profile, alignment rates across R1–R9) are reported as process evidence that the loop functions, labelled as such in every table, and excluded from confirmatory tests of H1d, H1e and H1f. **Exploratory (deferred):** co-occurrence-anomalous entailments, with a threshold sensitivity sweep. Not implemented in the initial battery; can be added post-pilot.

5.4 **Descriptive only:** cross-condition agreement; leave-one-out consensus recall, pooled and per-condition (R9 only).

5.5 **Statistical plan.** Mixed-effects models: B, D, and R fixed; seed random. R is a within-subject repeated measure with an AR(1) correlation structure. Binomial GLMM or beta regression for proportions; linear mixed-effects models for continuous scores.
...
- *B×D interaction:* H1c — the omnibus B×D term, then the pre-registered contrast (B2 − B0 | D0) − (B2 − B0 | D1). Simple effects of B within each D level are descriptive support, not the test.
...
Holm correction within families, applied to the designated primary outcomes; secondary outcomes are reported with effect sizes and intervals and are labelled as such. The R0-only coupled measures are a separate family.
...
7.15 **D1 integration analysis.** Integration loss by round and grounding level; LogMap alignment stability and IRI stability; the defect-routing split (single-source vs. multi-source) and what it implies about where decomposition's costs land.
...
8.1 **LLM-mediated merge (follow-on study).** The main design uses deterministic integration (union + LogMap). This ancillary study runs an LLM-mediated merge on B2D1 within seed and compares the result against the deterministic integration: does the LLM merge recover the integration loss, and at what cost? LogMap and AML agreement with the LLM merge decisions is reported.
...
8.4 **B0 spontaneous alignment.** BFO-only alignment rate and IOF alignment rate for B0 artefacts at R0, reported as a finding rather than a floor: it quantifies how much realist structure the generator supplies unprompted, and therefore how much of a contrast B0 really provides. IOF alignment in B0 and B1 ontologies (where IOF was not supplied) is itself informative about pretraining leakage.
...
5.8 *Rationale for homogeneous projection over heterogeneous methods, if needed.*
5.9 **Statistical plan.** Mixed-effects models: B, D, R fixed; seed random (with S-POP nested within S-GEN). Round is a within-subject factor with AR(1) correlation structure. RQ2.1–2.3, RQ2.5–2.7 run on all three grounding levels. RQ2.4 tested as a pre-registered regression of post-population reasoner-inconsistency rate on the Paper 1 structural quality vector (grounding-agnostic predictors, estimable on all 60 ontologies). The plateau-centre partition's community count and genre-AMI enter the same B/D/R mixed-effects model as additional confirmatory outcomes under RQ2.1–2.3, with yield included as a covariate to guard against size/density confounds. Modularity is not entered into this model; it is analysed separately in §7.3.
...
6.4 **RQ2.4: the structural-quality → inconsistency path.** Which Paper 1 measures predict post-population reasoner inconsistency? Estimated on all 60 ontologies.
...
10.3 Future work: the excluded CCO arm; held-out population corpora; multi-model replication; battery transfer to a second domain; BFO-specific modelling checks as a separate instrument publication.

---

## Parts V–VI — Implementation tasks

Moved to the companion file **dissertation_todo_v17.md**, which integrates the human-effort inventory and the sequenced next-actions list into a single task list with effort estimates and dependency tracking.
