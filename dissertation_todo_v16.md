...
4b. **Implement deterministic integration and defect routing for D1** — union of sub-ontology axioms plus LogMap alignment-based reconciliation (AML as audit comparison), IRI provenance map for defect routing, cross-genre defects routed to all involved sub-ontologies, round-to-round delta and IRI stability logged. No LLM merge call; no anchored re-integration needed (deterministic merge, same inputs → same output).
