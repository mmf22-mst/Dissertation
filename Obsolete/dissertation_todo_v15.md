...
Companion to **dissertation_overview_and_paper_outlines_v15.md**. Covers all three papers. All effort figures are authorship and implementation effort, not the campaigns themselves, which are sized in Part I §12.
...
3. **Implement structural profile computation** (OntoQA metrics; SPARQL or OWL API) and **set up the OOPS! pipeline** (web service or local scan). OOPS! is part of the feedback payload and is therefore loop-coupled by design. The full OOPS! report is fed back with no pitfall suppression; OntoQA is coupled through the OOPS! channel.
