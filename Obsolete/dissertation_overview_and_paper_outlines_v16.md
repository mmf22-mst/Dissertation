...
**Revision 16 — LLM-mediated merge replaced by deterministic union + LogMap alignment for D1 integration. Anchored re-integration, merge churn variance, pilot churn decision rule, and integration feedback channel all eliminated. Cross-genre defects route to all involved sub-ontologies. Per-round D1 compute drops from G+1 to G LLM calls. Integration loss measured honestly as the cost of decomposition. Prior revision history: v15 OntoQA reclassified as coupled, pitfall-overlap audit removed; v14 injection source and alignment-rate redesign; v13 E-code simplification; v12 reconciled against codebook v12; v11.4 D1 iteration on sub-ontologies with anchored re-integration; v11.3 three-allotment prompt decomposition; v11.2 corpus presentation order and deterministic windowing; v11.1 OOPS! retained in feedback loop with pitfall-overlap audit; v11 loop-coupled instruments identified and quarantined.**


*Body text presents the current design without backward references to prior revisions. Change logs are in the companion file **dissertation_change_logs_v16.md**.*
...
**Consequences for inference.** The coupled measures are global consistency, unsatisfiable-class count, OWL 2 DL conformance, OOPS! pitfall counts, OntoQA structural profile, and the two alignment rates. For these:
...
- **Confirmatory round-related inference rests on the held-out set:** CQ answerability, salient-term coverage, and genre coverage balance. The primary outcomes are all in this set, so the confirmatory core of H1d–H1f is unaffected by the coupling decision.
...
3.2 **Salient-term coverage** and genre coverage balance.
...
Moved to the companion file **dissertation_todo_v16.md**, which integrates the human-effort inventory and the sequenced next-actions list into a single task list with effort estimates and dependency tracking.
